...
class PatchLibrary:
    """Gère le stockage persistant, les métadonnées et le cache des rendus visuels."""
    def __init__(self, lib_path):
        self.lib_path = lib_path
        self.images_dir = os.path.join(lib_path, "images")
        self.heatmaps_dir = os.path.join(lib_path, "heatmaps")
    ...
    def add_patch(self, vector, source_img_path, coords, dino_version, input_size, heatmap_pixmap):
        """Ajoute un patch et génère son fichier de cache pour un affichage instantané."""
        patch_uid = str(uuid.uuid4())[:8]
        file_name = os.path.basename(source_img_path)
        
        # 1. Sauvegarde du rendu Heatmap en cache (PNG)
