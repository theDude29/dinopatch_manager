...
class PatchLibrary:
    """Gère le stockage persistant et la manipulation d'une base de données de patchs."""
    ...
    def add_patch(self, vector, source_img_path, coords, dino_version, input_size):
        """Ajoute un patch, copie l'image source et met à jour les vecteurs."""
    ...
    def remove_patch(self, index):
        """Supprime un patch par son index et nettoie les fichiers si nécessaire."""
    ...
    def save(self):
        """Sérialisation sur le disque (Auto-save)."""
    ...
    def load(self):
        """Désérialisation depuis le disque."""
    ...
    def merge(path_a, path_b, path_out):
        """Fusionne deux librairies existantes vers une nouvelle destination."""
        ...
        # Migration physique des images
        for lib_src in [lib_a, lib_b]:
            for img_name in os.listdir(lib_src.images_dir):
                shutil.copy2(os.path.join(lib_src.images_dir, img_name), 
                             os.path.join(lib_out.images_dir, img_name))
